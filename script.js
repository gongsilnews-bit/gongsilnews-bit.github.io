...
async function loadNews(category) {
    try {
        ...
        if (category === '찜한기사' || category === '최근본기사') {
            ...
            if (category === '전체기사' || category === '' || !category) {
        ...
        let finalNewsList = (rawArticles || []).map((a, i) => {
            // 실제 위도/경도만 사용 (없으면 null → 지도 마커 미표시)
            let mLat = a.lat || null;
            let mLng = a.lng || null;
...
window.showNewsDetail = async function(news) {
    ...
    // 사용자가 '예전 팝업창 스타일'을 삭제하고 모두 정상(새로운) 스타일로 가길 원하므로 무조건 이동
    const targetId = news.id || news.article_id;
    if (targetId) {
        window.location.href = `news_read.html?article_id=${targetId}`;
    }
};

window.closeNewsDetail = function() {
    // 팝업 닫기 기능은 더이상 쓰이지 않지만 호환성을 위해 유지
};
